...
class DataGenerator(Sequence):
    """Generates batches of training data, so all files don't have to be loaded
    into RAM at once"""

    def __init__(self, file_paths, labels, batch_size, num_classes,
                 shuffle=True):
...
# Build generators
batch_size = 10
train_gen = DataGenerator(train_paths, train_labels,
                          batch_size=batch_size, num_classes=len(label_map))
test_gen = DataGenerator(test_paths, test_labels, batch_size=batch_size,
                         num_classes=len(label_map), shuffle=False)

# Build model
model = build_baseline_cnn_model(input_shape=(
    128, 431, 1), num_classes=len(label_map))
